...
from db_soriana import ver_historia_compra
from datetime import datetime

def interfaz_historia_compra():
    ventana = Tk()
    ventana.title("Historial de Compras")
    ventana.geometry("1200x700")
    crear_seccion_historia_compra(ventana, None).pack(expand=True, fill="both")
    ventana.mainloop()

def crear_seccion_historia_compra(ventana, barra_lateral):
    campos = ["ID Compra", "Fecha", "Supervisor", "Artículo", "Cantidad", "Subtotal", "Método de Pago", "Nombre Proveedor", "ID Proveedor"]
    ...
    # Title on the left
    Label(frame_titulo_total, text="Historial de Compras", font=("Arial", 16, "bold"), bg="#E6F0FA").pack(side="left", pady=5)
    ...
    for col in campos:
        ...
        tabla.column(col, width=100 if col != "ID Proveedor" else 80)
    ...
    def actualizar_tabla(fecha, total_var):
        try:
            ...
            return
        success = ver_historia_compra(fecha, tabla)
        if success:
