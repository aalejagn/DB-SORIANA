...
def crear_seccion_metodo_de_pago(ventana, barra_lateral):
    ...
    Label(frame_search, text="Buscar por ID Método:", bg="#E6F0FA", font=("Arial", 12)).pack(side="left", padx=(10, 2))
    ...
    def buscar_y_mostrar():
        id_metodo = entry_busqueda.get().strip()
        if not id_metodo:
        ...
        resultado = buscar_metodo_de_pago(id_metodo)
        ...
        if resultado:
            ...
            id_metodo_original_var[0] = resultado[0]
        else:
            messagebox.showwarning("No encontrado", f"No se encontró un método de pago con el ID {id_metodo}")
